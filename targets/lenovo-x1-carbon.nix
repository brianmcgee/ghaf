...
# Generic x86_64 computer -target
{
  ...
  perSystem = {
    ...
    nixosConfigurations = let
      ...
      system = "x86_64-linux";
      formatModule = inputs.nixos-generators.nixosModules.raw-efi;
      mkNixosConfig = {extraModules ? []}: let
        netvmExtraModules = [
          ({pkgs, ...}: {
            microvm = {
              devices = lib.mkForce [
    ...
    in
      lib.mkIf (system == "x86_64-linux") {
        lenovo-x1-debug = mkNixosConfig {
          extraModules = debugModules;
        };
        lenovo-x1-release = mkNixosConfig {
          extraModules = releaseModules;
        };
        #        lenovo-x1-gnome-debug = mkNixosConfig {
        #          variant = "debug";
        #          extraModules = gnomeModules ++ debugModules;
        #        };
        #        lenovo-x1-gnome-release = mkNixosConfig {
        #          variant = "release";
        #          extraModules = gnomeModules ++ releaseModules;
        #        };
      };
  };
}
